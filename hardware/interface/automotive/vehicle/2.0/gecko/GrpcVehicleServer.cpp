...
class GrpcVehicleServerImpl : public GrpcVehicleServer, public vhal_proto::VehicleServer::Service {
  public:
    ...
    explicit GrpcVehicleServerImpl(const std::string& serverAddr, CommunicationThread* pthread)
        : mServiceAddr("127.0.0.1:50051"),
          mGarageModeHandler(makeGarageModeServerSideHandler(this, &mValueObjectPool, pthread)) {
    ...
    void setmCommThread(CommunicationThread* pthread) override;
...
GrpcVehicleServerPtr makeGrpcVehicleServer(const std::string& serverAddr,CommunicationThread* pthread) {

  return std::make_unique<GrpcVehicleServerImpl>(serverAddr,pthread); 
...
GrpcVehicleServer& GrpcVehicleServerImpl::Start() {
	...
	builder.AddListeningPort("127.0.0.1:50051", getServerCredentials());
...
void GrpcVehicleServerImpl::onPropertyValueFromCar(const VehiclePropValue& value,
                                                   bool updateStatus) {
    std::cout << "gecko" << __func__ <<  " prop: " << value.prop << std::endl;
...
StatusCode GrpcVehicleServerImpl::onSetProperty(const VehiclePropValue& value, bool updateStatus) {
    //此处添加往can网络下发命令的操作
    std::cout << "gecko" << __func__ << std::endl;
...
void GrpcVehicleServerImpl::setmCommThread(CommunicationThread* pthread){
    mCommThread = pthread;
    //mCommThread->getHandler()->getfunctionMap().propmanager.printProperties();
