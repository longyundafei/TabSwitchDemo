...
class PipeComm : public CommConn {
   public:
	...
	void sendOtaData(uint32_t addr,const uint8_t *data,uint32_t len) override;

	void sendOtaCtlCmd(uint8_t ctlCmd)override;
	
	void setOtaCallBack(const sp<IVehicleOtaCallback>& callback) override;
