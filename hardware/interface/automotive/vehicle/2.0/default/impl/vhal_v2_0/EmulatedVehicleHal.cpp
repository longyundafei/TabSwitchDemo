...
void EmulatedVehicleHal::onPropertyValue(const VehiclePropValue& value, bool updateStatus) {
    if(getValuePool() == nullptr) {
        ALOGE("getValuePool() is nullptr");
        return;
    }
    VehiclePropValuePtr updatedPropValue = getValuePool()->obtain(value);
//    int32_t propId = value.prop;
//    ALOGI("dxy ----- from mcu to soc  propId =  %d ", propId);
    if (mPropStore->writeValue(*updatedPropValue, updateStatus)) {
        getEmulatorOrDie()->doSetValueFromClient(*updatedPropValue);
        doHalEvent(std::move(updatedPropValue));
    }
}
StatusCode EmulatedVehicleHal::udsSendBack(const hidl_vec<uint8_t>& data)
{
    ALOGI("lihy ----- udsSendBack ... ");
    getEmulatorOrDie()->createUdsSendBackMsg(data);
    return StatusCode::OK;
}
