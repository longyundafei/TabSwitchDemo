...
#include "../../impl/vhal_v2_0/PipeComm.h"
...
public:
    VehicleHalManager(VehicleHal* vehicleHal,impl::PipeComm* pipeComm)
        : mHal(vehicleHal),
    ...
    Return<StatusCode> udsSendBack(const hidl_vec<uint8_t>& data)     override;
	Return<StatusCode> sendOtaData(uint32_t addr,const hidl_vec<uint8_t>& data, uint32_t len) override;
	Return<StatusCode> sendOtaCtlCmd(uint8_t ctlCmd) override;
	Return<StatusCode> setOtaCallBack(const sp<IVehicleOtaCallback>& callback) override;
