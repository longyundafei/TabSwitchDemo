 ...
 */
constexpr auto kMaxHidlVecOfVehiclePropValuePoolSize = 128;
...
Return<StatusCode> VehicleHalManager::udsSendBack(const hidl_vec<uint8_t>& data) {
    ALOGI("%s udsSendBack", __func__);
    mHal->udsSendBack(data);
    return StatusCode::OK;
}

Return<StatusCode>  VehicleHalManager::sendOtaCtlCmd(uint8_t ctlCmd){
	ALOGI("zwt spi VehicleHalManager sendOtaCtlCmd");
	mPipeComm->sendOtaCtlCmd(ctlCmd);
	return StatusCode::OK;
}

Return<StatusCode>  VehicleHalManager::sendOtaData(uint32_t addr,const hidl_vec<uint8_t>& data, uint32_t len) {
	ALOGI("zwt spi VehicleHalManager sendOtaData");
	mPipeComm->sendOtaData(addr,&data[0],len);
	return StatusCode::OK;
}

Return<StatusCode> VehicleHalManager::setOtaCallBack(const sp<IVehicleOtaCallback>& callback){
	ALOGI("zwt spi VehicleHalManager setOtaCallBack");
	mPipeComm->setOtaCallBack(callback);
	return StatusCode::OK;
}
...
void VehicleHalManager::onBatchHalEvent(const std::vector<VehiclePropValuePtr>& values) {
    ...
    for (const HalClientValues& cv : clientValues) {
        ...
        for (VehiclePropValue* pValue : cv.values) {
            ...
            if(pValue->prop == 557850895 || pValue->prop == 557850894 || pValue->prop == 289410563) {
               ALOGE("zhugang %d %d",pValue->prop, pValue->value.int32Values[0]);
            }
        }
